...
import 'package:crud_sqflite/crud_model.dart';
...
class DataBaseModel{
  ...
  Future<Database> intitDataBase() async {
    ...
    String path=join(documetDirectory.path.toString(),"cruds.db");
  ...
  Future addTask(CrudModel crudModel) async {
    Database db=await DataBaseModel .instance.database;
  ...
  Future updateData(CrudModel crudModel,int id)async{
    Database db=await DataBaseModel.instance.database;
    return await db.update("crud", crudModel.toJson());
  }
